                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'queryActivity':
                            ...
                            if(that.inputValue) {
                                if(joined) {
                                    expression = this.mac.fd.id + '=[' + that.curActivityIdArr + ']&&' + this.mac.fd.groupActivity.name + '="' + that.inputValue + '"&&(' + this.mac.fd.groupActivity.endTime + '<"' + curTime + '"||' + this.mac.fd.groupActivity.status + '=' + this.mac.enum.groupActivity.status.closed + ')';
                                } else {
                                    expression = this.mac.fd.id + '=[' + that.curActivityIdArr + ']&&' + this.mac.fd.groupActivity.name + '="' + that.inputValue + '"&&' + this.mac.fd.groupActivity.status + '=' + this.mac.enum.groupActivity.status.released;
                                }

                            } else {
                                if(joined) {
                                    expression = this.mac.fd.id + '=[' + that.curActivityIdArr + ']&&(' + this.mac.fd.groupActivity.endTime + '<"' + curTime + '"||' + this.mac.fd.groupActivity.status + '=' + this.mac.enum.groupActivity.status.closed + ')';
                                } else {
                                    expression = this.mac.fd.id + '=[' + that.curActivityIdArr + ']&&' + this.mac.fd.groupActivity.status + '=' + this.mac.enum.groupActivity.status.released;
                                }
                            }
