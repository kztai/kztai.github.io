    ...
    export default {
        ...
        data: function () {
            return {
                ...
                pvt_eventPortInput: ['placeRefreahEvent'],
        ...
        methods: {
            startModule: function (successCallBack, errorCallBack) {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            para.nStep = 'requireMap'
                            dbFlag += 1;
                            break
                        case 'mapURLQuery':
                            objectName = $this.mac.tb.groupInfo;
                            parentRecord = null;
                            condition = null;
                            dbData = {};
                            dbData[$this.mac.fd.groupInfo.mapURL] = [];
                            $this.sys.api.recordQuery(objectName, parentRecord, condition, dbData,
                                function () {
                                    para.mapURL = dbData[$this.mac.fd.groupInfo.mapURL][0]+'&callback=init';
                                    // para.nStep = 'requireMap'
                                    para.nStep = 'requireMap'
                                    dbFlag += 1;
                                    if (dbFlag === 2) {
                            ...
                            break
                        case 'requireMap':
                            para.mapURL = 'http://webapi.amap.com/maps?v=1.3&key=5af2887abea90c9ac6a3e02ed8b4f8b5&plugin=AMap.Geocoder&callback=init';
                            para.successCallBack();
                            window.init = function(){
                                para.nStep = 'placeQuery';
                                $this.startModule(successCallBack, errorCallBack)
                            };
                            var script = document.createElement('script')
                            script.type = 'text/javascript'
                            script.async = true
                            script.src = para.mapURL;
                            document.body.appendChild(script)
                            break
                        case 'placeQuery':
                            objectName = $this.para.mapData[0]['$table'];
                            parentRecord = null;
                            condition = null;
                            dbData = {};
                            dbData[$this.mac.fd.id] = [$this.para.mapData[0]['$arrValue'][0]];
                            dbData[$this.para.mapData[0]['$arrField'][0]] = [];
                            $this.sys.api.recordQuery(objectName, parentRecord, condition, dbData,
                                function () {
                                    para.nativePlace =  dbData[$this.para.mapData[0]['$arrField'][0]][0];
                        ...
                        case 'mapInit':
                            ...
                            $this.map = new AMap.Map('container', {
                                center: [para.nativePlace['lng'], para.nativePlace['lat']],
                                resizeEnable: true,
                                zoom: 15
                            })
                            ...
                            para.nStep = 'end'
                            para.result = para.nativePlace['place']
                            if (++dbFlag === 2) {
                                $this.startModule(successCallBack, errorCallBack);
                            }
                            break;
            ...
            initMap:function(successCallBack, errorCallBack){
                let $this = this;
                let para;
                let dbFlag;
                let objectName;
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'mapInit':
                            ...
                            //     $this.map && $this.map.destroy();
                            // }
                            $this.selectArea = $this.attr.area+$this.attr.place;
                            AMap.service(["AMap.PlaceSearch"], function () {
                                ...
                                // 添加列表点选监听事件
                                $this.placeSearch.search($this.selectArea, function (status, result) {
                                    if(status === 'complete'){
                                        ...
                                        if($this.map) {
                                            $this.map.setCenter( [para.lng, para.lat]);
                                            $this.marker.setPosition([para.lng, para.lat])
                                        }else{
                                            $this.map = new AMap.Map('container', {
                                                center: [lng, lat],
                                                resizeEnable: true,
                                    ...
                                    if (++dbFlag === 2) {
                                        $this.initMap(successCallBack, errorCallBack);
                                    }
                                })
                            });
                            break;
                        case 'placeQuery':
                            ...
                            $this.sys.api.recordQuery(objectName, parentRecord, condition, dbData,
                                function () {
                                    para.nativePlace =  dbData[$this.para.mapData[0]['$arrField'][0]][0];
                                    para.nativePlace['place'] = $this.attr.place;
                                    para.nativePlace['lng'] = para.lng;
                                    para.nativePlace['lat'] = para.lat;
                                    para.nStep = 'placeModify'
                                    if (++dbFlag === 2) {
                                        $this.initMap(successCallBack, errorCallBack);
                                    }
                                }, para.errorCallBack)
                        ...
                        case 'placeModify':
                            ...
                            dbData = {};
                            dbData[$this.mac.fd.id] = [$this.para.mapData[0]['$arrValue'][0]];
                            dbData[$this.para.mapData[0]['$arrField'][0]] = [para.nativePlace];
                ...
                $this.geocoder.getAddress(e.lnglat.getLng() + ',' + e.lnglat.getLat(), function (status, result) {
                    if (status === 'complete' && result.regeocode) {
                        ...
                        $this.infoWindow = new AMap.InfoWindow({
                            ...
                            content: $this.createInfoWindow(address, content, {
                                lng: e.lnglat.getLng(),
                                lat: e.lnglat.getLat()
                            }),
                        ...
                        // infoWindow.setContent('<p>'+address+'</p><button>保存站点</button>')
            ...
            createInfoWindow: function (title, content, titude) {
                let $this = this;
                //info 为 信息窗体
                ...
                var sharp = document.createElement("button");
                sharp.className = "info-bottom-button";
                sharp.innerHTML = '选为活动地点'
                sharp.onclick = function () {
                    var province = content['province'];
                    var city = content['city'];
                    var district = content['district'];
                    var area = province + city + district;
                    var place = $this.simplized(title.substring(area.length, title.length));
                    ...
                    if (content['citycode'] === '1852') {
                        province = $this.simplized(province);
                        district = $this.simplized(district);
                        if (district === '中西区' || district === '东区' || district === '南区' || district === '湾仔区') {
                            inputCity = '香港岛';
                        } else if (district === '九龙城区' || district === '观塘区' || district === '深水埗区' || district === '黄大仙区' || district === '油尖旺区') {
                            inputCity = '九龙';
                        } else {
                            inputCity = '新界';
                        }
                    } else if (content['citycode'] === '1853') {
                        province = $this.simplized(province);
                        district = $this.simplized(district);
                        if (district === '嘉模堂区' || district === '圣方济各堂区' || district === '路氹填海区') {
                            inputCity = '澳门离岛';
                        } else {
                            inputCity = '澳门半岛';
                        }
                    } else if (content['citycode'] === '1886') {
                        province = '台湾省'
                        inputCity = '台湾省';
                        district = '台湾省';
                    }
                    // $this.sm.smAddress.resetValue([province, inputCity, district]);
                    let data = {};
                    data['placeArr'] = [province, inputCity, district];
                    data['place'] = place;
                    data['lng'] = titude['lng'];
                    data['lat'] = titude['lat'];
                    $this.ep.placeRefreahEvent(data,function () {
                        $this.map.clearInfoWindow()
                        $this.map.setCenter([titude['lng'], titude['lat']]);
                        $this.marker.setPosition([titude['lng'], titude['lat']])
                    },function (error) {
                        console.error(error)
                    })
                };
                ...
                img.src = 'https://img-qn.hdb.com/static_v4/images/other/field_tc_bg.png';
            ...
            simplized: function (cc) {
                ...
                for (var i = 0; i < cc.length; i++) {
                    ...
                    else if (this.qqPYStr().indexOf(cc.charAt(i)) != -1)
                        str += this.simpPYStr().charAt(this.qqPYStr().indexOf(cc.charAt(i)));
                    else
                        str += cc.charAt(i);
                }
                return str;
