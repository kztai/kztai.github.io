    ...
    export default {
        ...
        methods: {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            ...
                            dbData[that.mac.fd.groupActivityTicketType.minNumber] = [];
                            dbData[that.mac.fd.groupActivityTicketType.maxNumber] = [];

                            that.sys.api.recordRead(tableName, dbData, function () {
                                para.ticketTotal = dbData[that.mac.fd.groupActivityTicketType.poll][0];

                                that.billMin = dbData[that.mac.fd.groupActivityTicketType.minNumber][0];
                                para.billMax = dbData[that.mac.fd.groupActivityTicketType.maxNumber][0];
                                ...
                                para.startTime = dbData[that.mac.fd.groupActivityTicketType.startTime][0].slice(0, -3);
                                para.endTime = dbData[that.mac.fd.groupActivityTicketType.endTime][0].slice(0, -3);
                                that.activityTime = para.startTime + ' ~ ' + para.endTime;

                                para.nStep = 'getApplyNum';
                                if (++dbFlag === 2) {
                        ...
                        case 'getApplyNum':
                            ...
                            that.sys.api.recordRead(tableName, dbData, function () {
                                let curApplyNum = dbData[that.mac.fd.groupActivity.enrollNumber][0][that.para.curTicketId - 1];
                                let ticketLeave = para.ticketTotal - curApplyNum;

                                if (para.billMax > ticketLeave) {
                                    that.billMax = ticketLeave
                                } else {
                                    that.billMax = para.billMax;
                                }
                        ...
                        case 'newOrder':
                            ...
                            dbData[that.mac.fd.groupActivityOrder.orderNumber] = [that.orderNumber];
                            dbData[that.mac.fd.groupActivityOrder.applicant] = [that.getCookie('userID')];
                            dbData[that.mac.fd.groupActivityOrder.name] = [that.getCookie('userName')];
            ...
            getBJTime: function () {
                ...
                request.open("GET", "http://bjtime.cn", false);
                request.setRequestHeader("If-Modified-Since", "bjtime");

                request.onload = function() {
                    if(request.status === 200) {
                        var data = JSON.parse(this.responseText);
                        if(data.success) {
                            document.getElementById('result1').innerHTML = data.msg;

                        } else {
                            document.getElementById('result1').innerHTML = "出现错误："+ data.msg;
