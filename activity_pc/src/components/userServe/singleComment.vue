<template>
    <!--<div class="comment-component-wrap" v-if="!(!isSelf && hideComment===mac.enum.userBlogComment.hideComment.yes) && !deleteDeep">-->
    <div class="comment-component-wrap">
        ...
        <div class="comment-content-wrap">
            ...
            <div class="comment-handle" v-if="isDelete === mac.enum.userBlogComment.delete.no && !isClose">
            ...
            <div class="comment-publish" v-if="isDelete === mac.enum.userBlogComment.delete.no && !isClose" v-show="showInput && !inputDisabled">
...
<script>
    ...
    export default {
                ...
                return {
                    nAuthorImgRef: {
                        para: {
                            ...
                            image: {
                                $table: {'userBlogComment': ['head_portrait']},
            ...
            pvt_commentInput_attr: function () {
                return {
                    commentInputRef: {
                        attr: {
                            // placeholder: '说点什么吧...',
                            // disabled: this.para.inputDisabled,
                            size: 'mini'
                        },
                    },
                };
        ...
        methods: {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            ...
                            that.isDelete = that.mac.enum.userBlogComment.delete.no;
                        ...
                        case 'getCommentData':
                            ...
                            dbData[that.mac.fd.userBlogComment.time] = [];
                            dbData[that.mac.fd.userBlogComment.observer] = [];
                            dbData[that.mac.fd.userBlogComment.praiseNumber] = [];
                            dbData[that.mac.fd.userBlogComment.delete] = [];
                            // dbData[that.mac.fd.userBlogComment.commentNumber] = [];
                            dbData[that.mac.fd.userBlogComment.hideComment] = [];
                            dbData[that.mac.fd.userBlogComment.userBlog] = [];
                            dbData[that.mac.fd.userBlogComment.parentID] = [];
                            dbData[that.mac.fd.userBlogComment.details] = [];

                            that.sys.api.recordRead(tableName, dbData, function () {
                                that.observerId = dbData[that.mac.fd.userBlogComment.observer][0];
                                that.hideComment = dbData[that.mac.fd.userBlogComment.hideComment][0];
                                that.curBlogId = dbData[that.mac.fd.userBlogComment.userBlog][0];
                                that.curParentId = dbData[that.mac.fd.userBlogComment.parentID][0];
                                that.commentText = dbData[that.mac.fd.userBlogComment.details][0];
                                that.isDelete = dbData[that.mac.fd.userBlogComment.delete][0];

                                if (dbData[that.mac.fd.userBlogComment.praiseNumber][0] > 0) {
                                    that.praiseNum = dbData[that.mac.fd.userBlogComment.praiseNumber][0];
                                }

                                // 判断时间的格式：
                                var time = dbData[that.mac.fd.userBlogComment.time][0];
                        ...
                        case 'parentCommentName':
                            ...
                            dbData[that.mac.fd.userBlogComment.name] = [];
                            that.sys.api.recordRead(tableName, dbData, function () {
                                that.parentCommentName = dbData[that.mac.fd.userBlogComment.name][0];
                                para.nStep = 'getCommentPraiseState';
                                if (++dbFlag === 2) {
                                    that.startModule(successCallBack, errorCallBack)
                                }
                        ...
                        case 'getCommentPraiseState':
                            ...
                            break;
                        case 'init':
                            if (that.hideComment === that.mac.enum.userBlogComment.hideComment.yes) {
                                that.isCloseStr = '已屏蔽';
                                if(!that.isSelf) {
                            ...
                            if(that.isDelete === that.mac.enum.userBlogComment.delete.yes) {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'dataModify':
                            ...
                            if(that.praiseNum === '点赞') {
                                dbData[that.mac.fd.userBlogComment.praiseNumber] = [0];
                            } else {
                                dbData[that.mac.fd.userBlogComment.praiseNumber] = [that.praiseNum];
                            }
            ...
            //     };
            //     if(that.isCloseStr !== '') {
            //         this.callModuleEventPort('setBlogIn', [this.mac.setBlog.type.showComment, commentData], function () {
            //             that.isCloseStr = '';
            //         }, function () {
            //         })
            //     }
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                            ...
                            that.$confirm('是否屏蔽该评论?', '提示', {
                                ...
                                if(that.isCloseStr !== '已屏蔽') {
                                    that.callModuleEventPort('setBlogIn', [that.mac.setBlog.type.hideComment, commentData], function () {
                                        ...
                                        para.isHide = that.mac.enum.userBlogComment.hideComment.yes;
                                        para.nStep = 'dataModify';
                                        if (++dbFlag === 2) {
                                ...
                                if(that.isCloseStr !== '') {
                                    that.callModuleEventPort('setBlogIn', [that.mac.setBlog.type.showComment, commentData], function () {
                                        ...
                                        para.isHide = that.mac.enum.userBlogComment.hideComment.no;
                                        para.nStep = 'dataModify';
                                        if (++dbFlag === 2) {
                                            that.closeComment(successCallBack, errorCallBack)
                                        }
                        ...
                        case 'dataModify':
                            ...
                            dbData = {};
                            dbData[that.mac.fd.id] = [that.para.curCommentId];
                            dbData[that.mac.fd.userBlogComment.hideComment] = [para.isHide];
                            that.sys.api.recordModify(tableName, dbData, function () {
                                para.nStep = 'end';
                                if (++dbFlag === 2) {
                                    that.closeComment(successCallBack, errorCallBack)
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'refreshCur':
                            ...
                            dbData= {};
                            dbData[that.mac.fd.id] = [that.para.curCommentId];
                            dbData[that.mac.fd.userBlogComment.delete] = [that.mac.enum.userBlogComment.delete.yes];
                        ...
                        case 'deleteInit':
                            that.callModuleEventPort('commentDeleteEvent', [that.para.curCommentId], function () {
                                that.commentText = '该评论已删除';
                                that.commentDelBtn = '删除';
                                that.isDelete = that.mac.enum.userBlogComment.delete.yes;
                                para.nStep = 'end';
                                if (++dbFlag === 2) {
                                    that.deleteCommentEvent(successCallBack, errorCallBack)
                                }
