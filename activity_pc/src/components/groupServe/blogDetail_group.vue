<template>
    <div id="group-blog-detail-wrap" class="screen-change">
        <header>
        ...
        <div class="content" :class="{'content-hide-right': !para.showUserInfo}" ref="detailContent">
            <div class="content-left">
                <div class="content-exist" v-if="!contentDelete">
                    <div class="blog-content">
                        <div v-if="blogType===mac.enum.groupBlog.blogType.article" class="content-title">
                        ...
                        <div class="blog-content-wrap">
                            <div class="content-author-info">
                                ...
                                <div class="group-name">
                                    <lm-link-text
                                            :para="pvt_groupName_para[pvt_currentRef.groupName].para"
                                            :attr="pvt_groupName_attr[pvt_currentRef.groupName].attr"
                                            :refId="pvt_currentRef.groupName"
                                            :ref="pvt_currentRef.groupName">
                                    </lm-link-text>
                                </div>
                                ...
                                <div class="attention-btn" v-if="showAttention" :class="{'is-attention': isAttention}">
                                <!--<div class="attention-btn" :class="{'is-attention': isAttention}">-->
                                    <lm-button
                            ...
                            <div class="content-text">
                                <div v-html="details" class="content-detail"></div>
                                <div v-if="blogType===mac.enum.groupBlog.blogType.post" class="content-picture">
                                    <lm-imgs
                        ...
                        <div class="handle-btns">
                            ...
                            <div v-if="isSelf" ref="moreBtn" class="more-btn">
                                <lm-button
                    ...
                    <div ref="blogComment" class="blog-comment">
                        ...
                        <div class="blog-comment-content-list" v-if="commentValueArr.length>0">
                            <div class="blog-comment-content-item" v-for="(item, index) in pvt_commentGroup_fordata[pvt_currentRef.commentGroup].data" :key="index">
                                <commentGroup
            ...
            <div class="content-right" v-if="para.showUserInfo">
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                pvt_vesselRef: {
                    vesselId: ['userInfo', 'foot', 'head', 'activity', 'apply', 'authorImg', 'authorName', 'releaseTime', 'deleteBtn', 'attentionBtn', 'title', 'picture', 'turn', 'commentBtn', 'praiseBtn', 'favoriteBtn', 'moreBtn', 'publishBtn', 'commentInput', 'closePop', 'deleteText', 'commentGroup', 'empty', 'groupName', 'blogSort', 'titleTime'],
                    ref: ['userInfoRef', 'footRef', 'headRef', 'activityRef', 'applyRef', 'authorImgRef', 'authorNameRef', 'releaseTimeRef', 'deleteBtnRef', 'attentionBtnRef', 'titleRef', 'pictureRef', 'turnRef', 'commentBtnRef', 'praiseBtnRef', 'favoriteBtnRef', 'moreBtnRef', 'publishBtnRef', 'commentInputRef', 'closePopRef', 'deleteTextRef', 'commentGroupRef', 'emptyRef', 'groupNameRef', 'blogSortRef', 'titleTimeRef']
                },
        ...
        computed: {
                ...
                return {
                    authorImgRef: {
                        para: {
                            ...
                            image: {
                                $table: {'groupInfo': ['logo']},
                                $value: [this.curGroupInfoId]
                            },
                        },
                    },
                };
            ...
            pvt_groupName_comp: function () {
                return {
                    currentRef: this.pvt_currentRef.groupName,
                    isShow: this.pvt_isShow.groupName,
                    groupNameRef: {
                        componentName: 'radioC',
                    },
                }
            },
            pvt_groupName_para: function () {
                return {
                    groupNameRef: {
                        para: {
                            $api: this.pvt_isSubModuleStarted,
                            $isShow: this.pvt_groupName_comp.isShow,
                            $isMounted: this.pvt_isMounted.groupNameRef,
                            textData: {
                                $table: {'groupInfo': ['group_name']},
                                $value: [this.curGroupInfoId]
                            },
                        },
                    },
        ...
        mounted: function () {
            ...
            this.moreBtn = this.$refs.moreBtn;
            window.scrollTo(0, 0);
        },
        methods: {
                    ...
                    for (let i = 0; i < tableIdArr.length; i++) {
                        if (parentIdArr[i] && parentIdArr[i] !== tableIdArr[i]) {
                            ...
                            commentDataArr.push({
                                ...
                                cloudBlogId: this.para.cloudBlogId,
                                children: []
                            });
                            tableIdArr.splice(i, 1);
                            parentIdArr.splice(i, 1);
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            ...
                            that.curGroupInfoId = null;
                            that.curGroupSite = '';
                            that.curGroupId = null;

                            that.isGroupMember = false;
                            that.isPraise = false;
                            that.isFavorite = false;
                            that.isAttention = false;
                            that.blogType = that.mac.enum.groupBlog.blogType.post;
                            that.details = '';
                            that.closeComment = that.mac.enum.groupBlog.closeComment.no;
                            that.releaseTime = '';
                            that.praiseNum = '点赞';
                            that.commentNum = '评论';
                            that.turnNum = '转发';
                        ...
                        case 'getGroupInfo':
                            ...
                            dbData = {};
                            dbData[that.mac.fd.id] = [that.curGroupInfoId];
                            dbData[that.mac.fd.groupInfo.groupID] = [];
                            that.sys.api.recordRead(tableName, dbData, function (result) {
                                that.curGroupId = dbData[that.mac.fd.groupInfo.groupID][0];
                                if(that.para.showUserInfo) {
                                    para.nStep = 'inputCloudClerkInfo';
                                } else {
                            ...
                            break;
                        case 'inputCloudClerkInfo':
                            that.callModuleEventPort('inputClerkInfo', [], function () {
                        ...
                        case 'inputTerClerkInfo':
                            expression = that.mac.fd.clerk.groupID + '=' + that.curGroupId + '&&' + that.mac.fd.clerk.user + '=' + that.myId;
                            start = null;
                            total = null;
                            sort = ['id,asc'];
                            destGeneSite = '';
                        ...
                        case 'inputBlog':
                            ...
                            that.sys.api.dataInput(tableName, [that.para.terBlogId], '', function (result) {
                        ...
                        case 'getBlogData':
                            ...
                            dbData[that.mac.fd.groupBlog.praiseNumber] = [];
                            dbData[that.mac.fd.groupBlog.commentNumber] = [];
                            ...
                            that.sys.api.recordRead(tableName, dbData, function (result) {
                                ...
                                para.details = dbData[that.mac.fd.groupBlog.details][0];
                                that.closeComment = dbData[that.mac.fd.groupBlog.closeComment][0];
                                that.authorName = '发表人：' + dbData[that.mac.fd.groupBlog.authorName][0];
                                that.authorId = dbData[that.mac.fd.groupBlog.author][0];
                        ...
                        case 'onloadComment':
                            // 是否允许评论：
                            if(that.closeComment === that.mac.enum.groupBlog.closeComment.yes) {
            ...
            inputComment: function(successCallBack, errorCallBack) {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            expression = that.mac.fd.groupBlogComment.groupBlogID + '=' + that.para.terBlogId + '&&' + that.mac.fd.groupBlogComment.parentID + '=[' + couldParentId + ']';
            ...
            joinCommentData: function (commentDataArr, successCallBack, errorCallBack) {
                ...
                rootComment:
                    for (let i = 0; i < commentDataArr.length; i++) {
                        ...
                        for (let j = 0; j < commentDataArr[i].children.length; j++) {
                            if (commentDataArr[i].children[j].isDelete === that.mac.enum.groupBlogComment.delete.yes) {
                                for (let m = 0; m < commentDataArr[i].children.length; m++) {
                                    if (commentDataArr[i].children[j].commentId === commentDataArr[i].children[m].parentId) {
                                        if (commentDataArr[i].children[m].isDelete === that.mac.enum.groupBlogComment.delete.no) {
                                            break;
                ...
                if (!that.isSelf) {
                    isHide:
                        for (let i = 0; i < commentDataArr.length; i++) {
                            ...
                            for (let j = 0; j < commentDataArr[i].children.length; j++) {
                                if (commentDataArr[i].children[j].isHide === that.mac.enum.groupBlogComment.hideComment.yes) {
                                    for (let m = 0; m < commentDataArr[i].children.length; m++) {
                                        if (commentDataArr[i].children[j].commentId === commentDataArr[i].children[m].parentId) {
                                            if (commentDataArr[i].children[m].isHide === that.mac.enum.groupBlogComment.hideComment.no) {
                                                break;
                                            } else if (m === commentDataArr[i].children.length - 1) {
                                                commentDataArr[i].children.splice(j, 1);
                                                j--;
            ...
            //                     blogData.userType = that.mac.userType.member;
            //                 } else {
            //                     blogData.userType = that.mac.userType.nonmember;
            //                 }
            //                 if (this.isPraise) {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'dataModify':
                            tableName = that.mac.tb.groupBlog;
                            dbData = {};
                            dbData[that.mac.fd.id] = [that.para.terBlogId];
                            if(that.praiseNum === '点赞') {
                                dbData[that.mac.fd.groupBlog.praiseNumber] = [0];
                            } else {
                                dbData[that.mac.fd.groupBlog.praiseNumber] = [that.praiseNum];
            ...
            deleteTextClick: function() {
                if(this.para.showUserInfo) {
                    this.callModuleEventPort('backEvent', ['public']);
                } else {
                    this.callModuleEventPort('backEvent', ['group']);
                }
            ...
            deleteBtnClick: function() {
                ...
                that.$confirm('此操作将删除该博文, 是否继续?', '提示', {
                    ...
                    this.callModuleEventPort('setBlogIn', [that.mac.setBlog.type.deleteBlog, blogData], function () {
                        that.contentDelete = true;
                        that.isAllComment = true;
                        ...
                        // 刷新终端数据：
                        let modifyData = {};
                        modifyData[that.mac.fd.id] = [that.para.terBlogId];
                        modifyData[that.mac.fd.groupBlog.status] = [that.mac.enum.groupBlog.status.delete];
                        that.callModuleEventPort('refreshTerminal', [that.mac.tb.groupBlog, modifyData], function () {
                        }, function () {
                        })
                    }, function (error) {
                        that.$message.warning('出错了！请稍后再试。');
                                ...
                                that.commentValueArr.unshift({
                                    para: {
                                        curCommentValue: {
                                            ...
                                            parentId: dbData[that.mac.fd.groupBlogComment.parentID][0],
                                            isDelete: dbData[that.mac.fd.groupBlogComment.delete][0],
                                            isHide: dbData[that.mac.fd.groupBlogComment.hideComment][0],
                                            cloudBlogId: that.para.cloudBlogId,
                                            children: []
                                        },
            ...
            commentGroupRef_inputNewComment: function (ref, cloudCommentId, successCallBack, errorCallBack) {
