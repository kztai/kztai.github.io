<template>
    <div id="my-draft-wrap">
        ...
        <div class="my-draft-list" v-if="blogIdArr.length>0">
            <div class="my-draft-item" v-for="(item, index) in pvt_draftC_fordata[pvt_currentRef.draftC].data"
                 :key="index">
                <draftC
        ...
        methods: {
            ...
            inputBlog: function (successCallBack, errorCallBack) {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        case 0:
                            expression = that.mac.fd.userBlog.status + '=' + that.mac.enum.userBlog.status.draft;
                            start = that.recommendStart;
                            total = that.inputTotal;
                            sort = ['id,desc'];
                            // sort = ['id,asc'];
                            destGeneSite = '';
                            portName = that.mac.tb.userBlog;
                            inputData = null;
            ...
            draftCRef_draftDelete: function () {
                this.refreshDraft();
            },
