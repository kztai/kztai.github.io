    ...
    export default {
        ...
        methods: {
                ...
                while (1) {
                    ...
                    switch (para.nStep) {
                        ...
                        case 'blogData':
                            ...
                            dbData[that.mac.fd.userBlog.name] = [];
                            dbData[that.mac.fd.userBlog.abstract] = [];
                            that.sys.api.recordRead(tableName, dbData, function () {
                                that.blogType = dbData[that.mac.fd.userBlog.blogType][0];
                                if(dbData[that.mac.fd.userBlog.blogType][0] === that.mac.enum.userBlog.blogType.post) {
                                    that.blogAbstract = dbData[that.mac.fd.userBlog.abstract][0];
                                } else {
                                    that.isBold = true;
                                    that.blogAbstract = dbData[that.mac.fd.userBlog.name][0];
                                }
            ...
            openBlogDetail: function () {
                this.callModuleEventPort('draftClickEvent', [this.blogType, this.para.curBlogId]);
            },
            ...
            deleteDraft: function() {
                ...
                that.$confirm('此操作将删除该草稿, 是否继续?', '提示', {
                    ...
                    that.callModuleEventPort('setBlogIn', [that.mac.setBlog.type.deleteBlog, blogData], function () {
                        that.callModuleEventPort('draftDelete', []);
                        that.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
