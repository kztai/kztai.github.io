...
ZeroRpcClient.prototype.remoteCall = function (server, arrFuncPara, timeout, successCallBack, errorCallBack) {
    ...
    if (!this.checkBlocked(server)) {
        console.warn(server + '服务器未阻塞，开始rpc远程调用');
        // 未阻塞，则调用RPC：
        this.zeroRemoteCall(server, arrFuncPara, timeout, successCallBack, function (error) {
            // 是阻塞错误吗?
            if (error && error.code === that.$rpcDef.rpcError_block) {
                console.warn(server + '服务器阻塞，阻塞标志置为1');
                // 设置阻塞标志
                that.setBlockFlag(server, that.$rpcDef.block_blocked, function () {
                    // 已阻塞，将远程调用保存在阻塞队列中
                    let isFull = that.pushRemoteCallIntoBlock(server, arrFuncPara, timeout, function () {}, errorCallBack);
                    // 判断阻塞队列是否已满：
                    if (isFull) {
                        let error = {};
        ...
        // 已阻塞，将远程调用保存在阻塞队列中
        let isFull = this.pushRemoteCallIntoBlock(server, arrFuncPara, timeout, successCallBack, errorCallBack);
        // 判断阻塞队列是否已满：
        if (isFull) {
            let error = {};
            error.code = that.$rpcDef.rpcError_block;
    ...
    while (1) {
        ...
        switch (para.nStep) {
            ...
            case 'localCall':
                ...
                // 终端函数调用
                para.comm.remoteCall_localCall("", '', para.arrCallFuncPara, function (remoteResult) {
                    // 获取回调函数
                    let funcCallBack = para.arrCallFuncPara[remoteResult.callbackIndex];
            ...
            case 'socketConnect':
                ...
                if (!serverSocket.isServerConnected(clientId)) {
                    ...
                    if (clientSocket) {
                        if (clientSocket.isDisconnected()) {   // 建立的socket连接，已经断线
                            para.errorCallBack()
                        } else {
            ...
            case "remoteCall_server":
                ...
                $p.instanceName = "";
                $p.funcName = '';
                $p.funcPara = para.arrCallFuncPara;

                // 云端函数调用
                para.comm.remoteCall_server_timeout($p, function (remoteResult) {
                    // 获取回调函数
                    console.warn('remoteCall_server_timeout成功！参数：' + JSON.stringify($p));
                    let funcCallBack = para.arrCallFuncPara[remoteResult.callbackIndex];
                    funcCallBack(remoteResult.remoteResult);
                    ...
                    console.warn('remoteCall_server_timeout失败！参数：' + JSON.stringify($p) + '；error: ' + JSON.stringify(error));
                    para.commErrorCallBack(error);
                });
                break;
            case 'end':
        ...
        switch (para.nStep) {
            case 0:
                ...
                if (blocked === 0) {  // 类型为清除阻塞
                    console.warn('收到目标服务器清除阻塞指令，开始执行当前服务器阻塞的RPC请求！');
            ...
            case 'clearBlocked':
                ...
                that.zeroRemoteCall(distServer, arrFuncPara, timeout, function (result) {
                    ...
                    // 是阻塞错误吗?
                    console.warn('目标服务器主动调当前服务器的清除指令出错：' + JSON.stringify(error));
                    if (error && error.code === that.$rpcDef.rpcError_block) {
                        // 设置阻塞标志
                        para.block.blocked = 1;
                        para.nStep = 'end';
                        console.warn('目标服务器又被阻塞！等待下一次目标服务器主动调当前服务器的清除指令');
                    } else {
...
* -------------------------------------------------------------------------------------------------------------//
* */
ZeroRpcClient.prototype.pushRemoteCallIntoBlock = function (server, arrFuncPara, timeout, successCallBack, errorCallBack) {
    let isFull;
    // 获取阻塞信息
    let obj = this.arrBlockQueue[server];
    // 队列是否满?
    if (obj.arrCallBack.length >= this.blockMax) {
        console.warn('当前服务器阻塞队列已满，错误回调给应用层！');
        isFull = 1;
    } else {
        console.warn('当前服务器阻塞队列未满，队列长度：' + obj.arrCallBack.length + '；将用户RPC请求保存到队列！');
        isFull = 0;
        let call = {};
        call.server = server;
