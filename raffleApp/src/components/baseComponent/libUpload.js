  ...
  while (1) {
    ...
    switch (para.nStep) {
      ...
      case 'getRoot':
        ...
        $this.sys.api.getRootRecord_(data.objectName[para.i], data.record[para.i],
          function (result) {
            para.root = result;
            para.nStep = 'dataOutput';
            dbFlag += 1;
      ...
      case 'dataOutput':
        let tableVariable = [{
          $geneAddr: $this.sys.api.geneAddr,
          $table: para.root.tableName,
          $arrField: null,
          $arrValue: para.root.arrRecord,
          $allFlag: [1, 1, 1, 0],
          $arrSubTab: null,
        }];

        let destGeneSite = null;
        $this.sys.api.dataOutput(tableVariable, destGeneSite, function (result) {
          para.i += 1;
          para.nStep = 'getRoot';
          dbFlag += 1;
          if (dbFlag === 2) {
...
libUpload.uploadData1 = function (data, successCallBack, errorCallBack) {
  var $this = this;
  var para;
  var dbFlag;
  var fnname = 'libUpload.uploadData';
  ...
  while (1) {
    ...
    switch (para.nStep) {
      ...
      case 'dataOutput':
        ...
        let tableVariable = [{
          $geneAddr: $this.sys.api.geneAddr,
          $table: data.objectName[para.i],
          $arrField: null,
          $arrValue: data.record[para.i],
          $allFlag: [1, 1, 1, 0],
          $arrSubTab: null,
        }];
