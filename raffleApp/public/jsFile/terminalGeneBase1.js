function $CTerminalGeneBase(that) {
    ...
    this.$geneInitLize(that.geneAddr, that.instruct, that.para, that.parentGeneAddr, that.parentServer, that.user);
        ...
        switch (para.nStep) {
            case 0:
                ...
                if (userInfo) {
                    ...
                    let parentGeneAddr = $this.$getUrlPara("parentGeneAddr");
...
$CTerminalGeneBase.prototype.$geneInitLize = function(geneAddr, instruct, parameter, parentGeneAddr, parentServer, user) {
    this.$para = parameter;
    ...
    this.$user = user;
    this.$group = this.$getUrlPara("group");
        ...
        switch (para.nStep) {
            case 0:
                ...
                let arrData = {
                    ...
                    [$this.$mac.fd.$l_geneLog.instructCode]: [$this.$instruct],
                ...
                that = {
                    geneObject: $this,
                    geneAddr: $this.$instruct.geneAddr,
                    server: $this.$instruct.server,
                    eventLog: $this.$instruct.eventLog,
                    index: $this.$instruct.index
                };
                para.actionObject = new $CGeneAction(that);
                $this.$actionTrack.index.push($this.$instruct.index);
                ...
                break;
            case "userLink":
                para.geneSet = $this.$getUrlPara("geneSet");
                let funcPara = [null, "userLink", [para.geneSet, $this.$geneAddr, $this.$server], null, 0, function(result) {
            ...
            case "socketConnect":// todo 建联
                ...
                break;
            case "dataTransOpen":
                $this.$instruct.index.number = $this.$instruct.number;
                $this.$zDB.dataTransOpen($this.$dbName, $this.$instruct.index, function(result) {
            ...
            case "actionObject.portInitlize":
                ...
                para.eventPort = $this.$getUrlPara("eventPort");
                if (para.eventPort) {
                    $this.$instruct = $this.$getUrlPara("instruct");
                    dbResult = $this.$db.cacheColRead($this.$cloudDBName, $this.$mac.tb.eventPort, para.eventPort);
            ...
            case "actionObject.eventRunSub":
                para.actionObject.$eventRunSub(para.event, $this.$instruct, $this.$para, para.command, function() {
                    para.nStep = "end";
                    if (++dbFlag === 2) {
                        $this.$geneRun(successCallBack, errorCallBack);
                    }
        ...
        switch (nStep) {
            ...
            case "querySubDataPort":// 获取dataPortDeploy
                ...
                dataPortDeploy = {
                    ...
                    parentDataPort: Array.from(dbRecord.arrNumber, v => dbRecord.table[this.$mac.fd.subDataPort.connectParentdata][v] ? dbRecord.table[this.$mac.fd.subDataPort.dataportName][v] : null),
