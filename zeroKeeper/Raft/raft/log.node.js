...
node.log_node_add_new_entry = function(req, successCallBack, errorCallBack){
  ...
  if(!arrEntry || arrEntry.length === 0 || this.is_log_exist(zeroEntry)){
    ...
    if(zeroEntry && zeroEntry.type === 2 && zeroEntry.cmdIndex === null && this.arrLog[zeroIndex].fromNode !== this.nodeId){ // 20210112 毛光华要求添加
  ...
  if(this.haveLeader){
    // 详细设计修改 - 20210117
    req.consumeIndex = this.consumeIndex;
...
node.log_node_add_new_entry_response = function(req, res, successCallBack, errorCallBack){
  // 20210107
  // 20210128
  this.log_node_proc_consume_index(res.clearConsumeIndex);
  ...
  if(zeroEntry.type === 2 && zeroEntry.cmdIndex === null && res.fromNode !== this.nodeId){ // 20210112 毛光华要求添加
    successResult = false;
  }
 ...
 * 20210128
 */
node.log_node_proc_consume_index = function(clearConsumeIndex){
  if(clearConsumeIndex === 0) return;
  if(clearConsumeIndex !== this.consumeIndex) return;
  this.zk_clear_consume_index(clearConsumeIndex);
};
