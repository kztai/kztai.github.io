...
leader.log_leader_become_leader_init = function (olderLeader) {
  ...
  if (olderLeader !== this.nodeId) {
    ...
    //20210128
    this.clearConsumeIndex = 0;
    this.consumeIndexCount = 0;
...
leader.log_leader_add_new_entry = function (req, successCallBack, errorCallBack) {
  ...
  // 20210128
  this.log_leader_proc_consume_index(req.sendNode, req.consumeIndex);
...
leader.log_leader_sync_new_entry = function (oldReq, successCallBack, errorCallBack) {
  ...
  for (let i = 0; i < this.nodeCount; i++) {
    ...
    if (this.nodeId === nodeId) {
      // 20210128
      this.log_node_proc_consume_index(this.clearConsumeIndex);
      this.log_leader_proc_consume_index(this.nodeId, this.consumeIndex);
...
leader.log_leader_sync_new_entry_response = function (node, res, successCallBack, errorCallBack) {
  ...
  if (res.procConsumeIndex === 1) {
    ...
    this.log_leader_proc_consume_index(node, res.consumeIndex);
  }
...
leader.log_leader_get_sync_info = function (req) {
  ...
  req.confirmIndex = this.confirmIndex;
  // 20210128
  req.clearConsumeIndex = this.clearConsumeIndex;
...
leader.log_leader_proc_term = function (req) {
  ...
  if (req_term !== this.term) {
    if (this.term < req_term) {
      ...
      this.log_leader_close_consume_index_timer();
 ...
 * 20210128
 */
leader.log_leader_create_consume_index_timer = function(consumeIndex){
  let nTimeout = 1000 * 60 * 1;
  // let nTimeout = 1000;
  this.m_nConsumeIndexTimerNo = setTimeout(()=>{
    this.clearConsumeIndex = consumeIndex;
  }, nTimeout)
};

leader.log_leader_close_consume_index_timer = function () {
...
leader.log_leader_proc_consume_index = function (node, consumeIndex) {
  if(this.m_nConsumeIndexTimerNo === null){
    if(consumeIndex === 0) return;
    if(consumeIndex < this.consumeIndexCount) return;

    if(this.consumeCount === 0){
      // 第一次
      this.consumeIndexCount = consumeIndex;
      this.arrConsumeNode = {};
      this.consumeCount = 0;
      this.arrConsumeNode[node] = 1;
      this.consumeCount++;
    } else {
      if(consumeIndex === this.consumeIndexCount){
        if(!this.arrConsumeNode[node]){
          this.arrConsumeNode[node] = 1;
          this.consumeCount++;
        }
      } else {
  ...
  if(this.consumeCount !== this.nodeCount) return;
  this.log_leader_create_consume_index_timer(consumeIndex);
};
