...
base.raft_init = function (successCallBack, errorCallBack) {
  ...
  this.zk_sync_zk_node_to_log((res) => {
    ...
    if (res.arrEntry instanceof Array) {
      let entry, index;
      for(let i = 0; i < res.arrEntry.length; i++){
        entry = res.arrEntry[i];
        index = entry.index;
        // 删除日志中的索引
        delete entry.index;
        this.arrLog[index] = entry;
        this.arrLogIndex[this.get_log_id(entry)] = index
      }
    } else {
     ...
     * by pdr 20210107
     */
    this.consumeIndex = !!res.consumeIndex ? res.consumeIndex : -1; // 此处必须是小于0，否则会出现 zookeeper没有更新索引的情况下，raft先更新了0
...
base.raft_init_global_var = function (options) {
   ...
   */
  this.consumeIndex = 0;
  this.clearConsumeIndex = 0; // 20210128
  this.consumeCount = 0;
  this.consumeIndexCount = 0;

  this.m_nConsumeIndexTimerNo = null; // 计数器
  // this.zk_clear_consume_by_minIndex = options.zk_clear_consume_by_minIndex; // 清除已消费的zk节点轨迹记录 方法名
  this.zk_clear_consume_index = options.zk_clear_consume_index; // 清除已消费的zk节点轨迹记录 方法名
  // 临时修改
  // this.minConsumeIndexWait = 0;
};
