    ...
    while (1) {
        ...
        switch (para.nStep) {
            ...
            case 'get_mysql_data':
                ...
                this.$db.hardDirectRecordQuery(this.dbName, tableName, condition, arrField, dbData, para.arrFuncField, para.hardTrans, function ($result) {
                    para.dbData = dbData;
            ...
            case 'init_raft':
                this.zk_sync_zk_node_to_log(function (result) {
                    ...
                    // let arrIp = that.getServerIP(['192.168.1.117:58100', '192.168.1.117:58101', '192.168.1.117:58102']);
                    let arrNode = [];
                    ...
                    // 开关控制是否打印raft日志信息：
                    if ($zkDef.switch) {
                        options.logPath = global.path_zeroKeeper + '/Raft/logs/';  // 打印的错误信息存放地址
                        options.arrAllowLogType = ['sys', 'met-test', 'sys-test'];  // 允许打印的错误类型
                    }
...
ZKClient.prototype.zk_consume_entry = function (req, successCallBack, errorCallBack) {
    ...
    while (1) {
        ...
        switch (para.nStep) {
            case 0:
                if (req.confirmIndex > $this.me.confirmIndex) {
            ...
            case 'getConfirmEntry':  // 获取日志信息(多条)
                $this.getConfirmEntry($this.me.confirmIndex, req.confirmIndex, para.arrEntry);

                para.nStep = 'saveNode';
                dbFlag++;
                break;
            case 'saveNode':  // 保存节点数据以及轨迹到数据库
                $this.zk_save_zk_node(req, para.arrEntry, function (result) {
                    para.nStep = 'end';
                    if (++dbFlag === 2) {
                        $this.zk_consume_entry(req, successCallBack, errorCallBack)
                    }
                }, function (error) {
                    // 失败回调中的错误信息，需要回给应用层：
                    error.cmdId.forEach(function (item) {
                        let existInfo = cmdCallbackExist(item);
                        if (existInfo && $this.me.commitCmdObj[existInfo.commitId]) {
                            $this.me.commitCmdObj[existInfo.commitId].errorCallBack(error.error);
                            delete $this.me.commitCmdObj[existInfo.commitId];
                        }
                    });
...
ZKClient.prototype.create = function (pathArr, cmdIndex, dataArr, attribArr, successCallBack, errorCallBack) {
    let that = this;
    if ($zkDef.switch) console.log(global.localIp + '：create start');
    ...
    if (cmd) {
        if (cmd.arrEntry.length > 0) {
            that.log_zk_commit_cmd(cmd, function () {}, errorCallBack);
        } else {
...
ZKClient.prototype.setData = function (pathArr, cmdIndex, dataArr, versionArr, successCallBack, errorCallBack) {
    let that = this;
    if ($zkDef.switch) console.log(global.localIp + '：setData start');
    ...
    if (cmd) {
        if (cmd.arrEntry.length > 0) {
            that.log_zk_commit_cmd(cmd, function (isSelfCmd) {
                if(isSelfCmd === false) {
                    let error = {code: '03_33_0010', msg: '该日志已存在且不是本服务器创建！'};
                    let result = {};
                    result.arrId = [];
                    result.arrError = [];
        ...
        switch (para.nStep) {
            case 0:
                ...
                if (versionArr) {
                    ...
                    para.nStep = 'create_checkParentPath';
                }
                dbFlag++;
                break;
                    ...
                    if (type === $zkDef.cmd_create) {  // create类型：先判断父节点，再判断节点，最后判断命令是否存在：
                        if (para.parentPathExist[index]) {
                            ...
                            if (para.cmdIdObj[entry.cmdId]) {
                                ...
                                para.cmd.result.arrError[index] = null;
                            } else {
                                // 再判断节点是否已存在：
                                if (para.pathExist[index]) {
                                    // 节点已被创建，返回出错信息：
                                    para.cmd.result.arrId[index] = null;
                                    para.cmd.result.arrError[index] = {code: '03_33_0003', msg: '节点已存在！'};
                                } else {
                            ...
                            // 父节点不存在，返回出错信息：
                            para.cmd.result.arrId[index] = null;
                            para.cmd.result.arrError[index] = {code: '03_33_0005', msg: '父节点不存在！'};
                        }
                    } else {
                        ...
                        if (para.pathExist[index]) {
                            ...
                            if (para.cmdIdObj[entry.cmdId]) {
                                ...
                                if (versionArr[index] !== para.verData[index]) {
                                    para.cmd.result.arrId[index] = null;
                                    para.cmd.result.arrError[index] = {code: '03_33_0004', msg: '版本号错误！'};
                                } else {
                            ...
                            // 节点不存在，返回出错信息：
                            para.cmd.result.arrId[index] = null;
                            para.cmd.result.arrError[index] = {code: '03_33_0008', msg: '该节点不存在！'};
                        }
                    }
                });
        ...
        switch (para.nStep) {
                ...
                for (let i = 0; i < para.$arrRecordNo.length; i++) {
                    ...
                    if (para.cacheResult.code === 0) {
                        ...
                        if (para.cacheResult.code === $zkDef.errorCode_dupValue) {
                            ...
                            let err = {};
                            err.code = '03_33_0003';
                            err.message = '该路径已存在！（不写节点表，但是会新增历史记录）';

                            // 将错误信息写入缓冲区中对应的命令：
                            writeEntryError(curCmdId, err);
                        } else {
            ...
            case 'nodeModify':   // 获取到所有修改命令的nodeId与版本号，并判断版本号是否相同，相同则修改数据库中数据
                ...
                for (let i = 0; i < para.$arrRecordNo.length; i++) {
                    ...
                    if (dbVersion === userVersion) {
                        ...
                        let error = {};
                        error.code = '03_33_0004';
                        error.message = '版本号不相同';
                        arrSetRecord[i].verError = error;

                        // 将错误信息写入缓冲区中对应的命令：
                        let curCmdId = getCmdId(arrSetRecord[i]);
                        writeEntryError(curCmdId, error);
                    }
                }
            ...
            case 'makeMysqlData':
                ...
                for (let i = 0; i < arrSetRecord.length; i++) {
                    if(!arrSetRecord[i].indexError && arrSetRecord[i].cmdIndex !== null) {
                ...
                for (let paraCmdId in para.cmdIdObj) {
                    ...
                    if (arrIdIndex !== undefined) {
                        ...
                        if (entryError) {
                            ...
                            that.me.commitCmdObj[para.commitResult.commitId].result.arrId[arrIdIndex] = para.cmdIdObj[paraCmdId];
                            that.me.commitCmdObj[para.commitResult.commitId].result.arrError[arrIdIndex] = null;
                            ...
                            if (watcherFun) {
                                ...
                                watchResult.cmdIndex = that.$raft.arrLog[para.startIndex].cmdIndex;
                ...
                for (let i = startIndexArr[para.i] + 1; i <= endIndexArr[para.i]; i++) {
                    if (para.execFlagObj[i] === 0) {
                        ...
                        if (watcherFun) {
                            ...
                            watchResult.cmdIndex = entry.cmdIndex;
                            watcherFun(watchResult);
                        }
                    }
                }
    ...
    while (1) {
        ...
        switch (para.nStep) {
            ...
            case 'leaderModify_mem':
                ...
                para.cacheResult = $this.$db.cacheRecordModify($this.dbName, tableName, arrId, field, dbData);
                if (para.cacheResult.code === 0) {
            ...
            case 'leaderModify_mysql':
                ...
                $this.$db.hardDirectRecordModify($this.dbName, tableName, arrId, field, dbData, para.hardTrans, function (result) {
                    para.nStep = 'end';
                    if (++dbFlag === 2) {
                        $this.zk_db_save_leaderInfo(leaderData, successCallBack, errorCallBack)
                    }
                }, para.errorCallBack);
                break;
            case 'end':
    ...
    while (1) {
        ...
        switch (para.nStep) {
            ...
            case 'getLeaderData':
                ...
                para.cacheResult = $this.$db.cacheRecordRead($this.dbName, tableName, arrId, field, dbData);
                if (para.cacheResult.code === 0) {
            ...
            case 'getIndex':  // 获取确定指针，清除指针
                ...
                para.cacheResult = $this.$db.cacheRecordQuery($this.dbName, tableName, condition, field, dbData);
                if (para.cacheResult.code === 0) {
            ...
            case 'logInit':
                ...
                para.cacheResult = $this.$db.cacheMultiQuery($this.dbName, tableName, field, condition, $start, $total, $arrSort, dbData);
                if (para.cacheResult.code === 0) {
...
function cmdCallbackExist(cmdId) {
    for (let key in $this.me.commitCmdObj) {
        let commitId = key;
        let uniqueObj = $this.me.commitCmdObj[key];
